...
public class Solution {
    ...
    public static Direction lastDirection(LeePoint nextMove, Board board){
    ...
    public static Direction solve(Board board){
        ...
        if(lee.trace(start, apple, obstacles).isPresent()){
           ...
           return lastDirection(nextMove,board);
        }
        else if(board.getSnake().size() >= 60 && lee.trace(start, stone,
                board.getBarriersWithoutStone()).isPresent()){
